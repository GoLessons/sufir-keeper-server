...
func TestUpdateItemIntegration(t *testing.T) {
	...
	userID := uuid.New()
	uniqueLogin := "update_item_user_" + uuid.New().String()
	...
	// update
	updateHandler := NewUpdateHandler(items, kek, tokenAuth)
	var newData apitypes.ItemUpdate_Data
	require.NoError(t, newData.FromCredentialData(apitypes.CredentialData{Login: "user@example.com", Password: "newpass", Type: "CREDENTIAL"}))
	newTitle := "NewTitle"
	updBody := apitypes.ItemUpdate{Title: &newTitle, Data: &newData}
	updBytes, err := json.Marshal(updBody)
	require.NoError(t, err)
	updReq := testutil.AuthorizedJSONRequest(http.MethodPut, "/items/"+itemID.String(), userID, updBytes)
	updRec := httptest.NewRecorder()
	updateHandler.Handle(updRec, updReq, itemID)
